...
import {
  buyPumpFunTokens,
  sellAllPumpFunTokensFromMultipleWalletsInstruction,
  sellPumpFunTokens,
  setupPumpFunToken,
...
program.command('initMarketMakingWallets').action(async () => {
...
program.command('initMarketMakingService').action(async () => {
  const botCustomer = await getBotCustomerByName('CATF')
  const newTokenMint = '7uCHQdxAz2ojRgEpXRHas1nJAeTTo9b7JWNgpXXi9D9p'
  ...
  console.log(`created market making cycle ${cycle.id} for market making, isActive: ${shouldBeActive}`)
...
program
  ...
  program.command('executeRaydiumSwapForMarketMaking').action(async () => {
    const botCustomer = await getBotCustomerByName('CATF')

    const activeBookedService = await getActiveBookedServiceByBotCustomerId({
      botCustomerId: botCustomer.id,
      serviceType: EServiceType.MARKET_MAKING
    })
      ...
      await executeAndParseSwap(
        {
          ...
          maxSlippage: 500,
          poolId: new PublicKey(activeBookedService.poolIdForService!),
          poolSource: 'Raydium',
        },
        decryptedWallet,
      )
  ...
  program.command('executeJupiterSwapForMarketMaking').action(async () => {
...
program
  ...
  .action(async (opts) => {
    ...
    console.log(
      `settings -> walletCount: ${walletCount}, fundingCycles: ${fundingCycles}, minAmount: ${minAmount}, maxAmount: ${maxAmount}`,
    )
    ...
    for (const wallet of serviceFundingWallets) {
      ...
      if (balance > minAmount * LAMPORTS_PER_SOL) {
        console.log(
          `taking service funding wallet with balance > 0.1 SOL: ${wallet.pubkey} => balance: ${balance / LAMPORTS_PER_SOL} SOL`,
        )
        serviceFundingWalletsWithBalance.push(wallet)
      } else {
        console.log(
          `skipping service funding wallet with balance <= 0.1 SOL: ${wallet.pubkey} => balance: ${
...
program
  ...
  .action(async (opts) => {
    ...
    console.log(
      ...
      `(~${(tokenBalance * birdEyeUsdcRate).toFixed(2)} USDC / ~${((tokenBalance * birdEyeUsdcRate) / solUsdcRate).toFixed(
        2,
      )} SOL)`,
    )
  })
  ...
  for (const privateKey of privateKeys) {
    ...
    await reattempt.run({ times: 10, delay: 1000 }, async () => {
      ...
      if (confirmedResult.value.err) {
        console.log(
          `Error closing ${fromKeypair.publicKey.toBase58()} and funding ${toKeypair.publicKey.toBase58()}: ${
      ...
      console.log(
        ...
        txSig,
        `to close ${fromKeypair.publicKey.toBase58()} and fund ${toKeypair.publicKey.toBase58()} with ${
          balance / LAMPORTS_PER_SOL
        } SOL`,
      )
    })
...
program.command('fundWalletFromAllWallets').action(async () => {
  ...
  console.log(
    `Total balance: ${await getBalanceFromWallets(botCustomerWallets.map((wallet) => new PublicKey(wallet.pubkey)))} SOL`,
  )

  await asyncBatch(
    botCustomerWallets,
    async (botCustomerWallet) => {
...
program.command('setupPumpFun').action(async () => {
  ...
  let activeBookedService = !!customer
    ? await getActiveBookedServiceByBotCustomerId({ botCustomerId: customer.id, serviceType: EServiceType.SNIPER })
    : null
  ...
  if (!customer) {
    ...
    const { balanceFound, balance } = await waitUntilBalanceIsGreaterThan({
      from: mainWalletForNewCustomerKeypair.publicKey,
      amount: 0.1,
      waitTime: 20000,
    })
