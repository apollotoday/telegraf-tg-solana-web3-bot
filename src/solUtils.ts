...
import { primaryRpcConnection, primaryStakedRpcConnection } from './config'
...
export async function sendSol(args: { from: Keypair; to: PublicKey; amount: Sol; feePayer?: Keypair }) {
  ...
  return await sendAndConfirmVersionedTransactionAndRetry({ transaction, useStakedRpc: false })
}
...
export async function closeWallet(args: { from: Keypair; to: Keypair; feePayer?: Keypair; waitTime?: number }) {
  ...
  return await sendSol({
    from: args.from,
    to: args.to.publicKey,
    amount: Sol.fromLamports(balance),
    feePayer: args.feePayer ?? args.to,
  })
}
