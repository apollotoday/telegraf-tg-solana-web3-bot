import {
  ...
  import { buyAmount, primaryRpcConnection, walletCountsPerCycle } from '../../config';
  ...
  export async function rankingBoost(
    ...
    while (true) {
      const buyCounts = Math.min(Math.floor(Math.random() * walletCountsPerCycle), wallets.length)
      ...
      while (i++ < buyCounts) {
        ...
        // const buyAmount = Math.random() * (maxSol - minSol) + minSol
        // buyAmounts.push(buyAmount)
      }
      
      // await Promise.all(buyersWallet.map(async (buyer, idx) => {
    ...
    // const seller = buyersWallet[Math.floor(Math.random() * wallets.length)]
    // const tokenAta = getAssociatedTokenAddressSync(new PublicKey(token), new PublicKey(seller.pubkey))
    // const tokenBalInfo = await primaryRpcConnection.getTokenAccountBalance(tokenAta)
    // const tokenBalance = tokenBalInfo.value.amount
    // const wallet = Keypair.fromSecretKey(Uint8Array.from(JSON.parse(seller.encryptedPrivKey)))
    // const tx = await getSellTx(primaryRpcConnection, wallet, new PublicKey(token), NATIVE_MINT, tokenBalance, pool)
    // if (tx) {
    //   const sig = await primaryRpcConnection.sendTransaction(tx)
    //   await primaryRpcConnection.confirmTransaction(sig)
    // }
