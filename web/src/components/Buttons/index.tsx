...
export const WhiteButton: React.FC<WhiteButtonProps> = ({
  ...
  return (
    <Button
      ...
      borderRadius={'100px'}
      bg={isHovered ? 'linear-gradient(90deg, #0F0E0E -6%, #494949 45%, #0C0909 83%, #000000 100%)' : 'white'}
...
export const NoBgButton: React.FC<NoBgButtonProps> = ({ width, height, children, ...props }) => {
  return (
    <Button
      w={width}
      h={height}
...
export const GetStartedButton = ({ isMain = false, ...props }: GetStartedButtonTypes) => {
  return (
    <Button
      w={'full'}
      h={'44px'}
      ...
      {...props}
    >
      <Flex flexDir={'row'} justifyContent={'space-between'} w={'full'} alignItems={'center'}>
        <Box color={isMain ? 'black' : 'white'} fontWeight={600} lineHeight={'24px'} fontSize={'16px'}>
          Get Started
        </Box>
